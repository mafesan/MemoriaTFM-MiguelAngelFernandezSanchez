...
day using interactive applications which allows us to participate somehow into a social network. As a result of handling these
...
\begin{itemize}
    ...
    \item \textit{Retweet} a \textit{tweet} (propagate a message among all followers of the person who \textit{retwitted}).
...
After that, I'll detail the designing process and the tool architechture, itemizing on each one of its sections. Then,
we'll explore the performanc of the tool within an use case, taking into account the limitations and issues which arose
