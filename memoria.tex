...
obtained with this tool and its subsequent analysis of this data led to several scientific papers published on international Software Engineering conferences,
...
\label{sec:structure}
In this thesis are presented the main objectives which were marked to build this tool in chapter~\ref{sec:objectives},
followed by a brief explanation of the technology which have been used to achieve these objectives in chapter~\ref{sec:state-art}.
Next, in chapter~\ref{sec:design-implementation} it is detailed the designing process and the tool architecture, itemizing on each one of its components.
Thenceforth, the performance and results of the tool are explored in chapter~\ref{sec:results} by means of two different use cases,
...
Furthermore, some conclusions are presented in chapter~\ref{sec:conclusions}, reviewing whether the marked objectives have been accomplished or not,
along with a brief discussion about the limitations of this tool and future work.\\\\
This thesis contains the following annexes:
\begin{enumerate}[A.]
  \item Definitions
...
\begin{itemize}
  ...
  \item OB4: Analyze positive GitHub repositories (with at least one positive result) to obtain enhanced data from them.
...
GitHub allows to obtain its information through its \textit{API}\footnote{See the definition of GitHub
\textit{API} in section~\ref{sec:api}},
but this entails several limitations, for instance, the GitHub API has a maximum rate of 5.000 request per hour with each account.
Additionally, GitHub is not static (new repositories are added, existing ones are modified, etc.) and there isn't a direct way
of obtaining the amount of repositories in the platform at a certain date using its \textit{API}.
These limitations led to the decision of using the \emph{GHTorrent} project, which offers a queriable, offline database with most
of the information which the GitHub API can provide.\\\\
...
OB4 and OB5 have to be completed to answer \emph{RQ2}. Analyzing positive repositories is not a trivial task, as it is crucial
