...
very difficult to have quantitative and reliable data. For example, to build the graph like the one showed at
~\ref{fig:total-repo-number}, its numbers had to be obtained directly from articles written by the \emph{GitHub} team itself [\textcolor{red}{Source}]
...
for researchers, otherwise it would be practically non-viable retrieving full-scale \emph{GitHub} data. Nevertheless, it is important
to mark that \emph{GHTorrent} database dump \underline{does not contain} \textit{Git trees} (file-related) information:
if this information was included, a major part of this tool (and by extension, this project) wouldn't be necessary, as we could
query and filter the data of our interest directly from the database which the \emph{GHTorrent} project provides.\\
