...
the next goal to define is to obtain the whole dataset of repositories hosted on all GitHub at a certain date.\\
...
forward. Currenty, GitHub is the largest host of source code in the world,
  ...
  \item \textbf{Data extraction}
    \begin{itemize}
      \item \textbf{Extract file list}:
            ...
            (for each repository in the list from last section) quering the GitHub \textit{API}.
...
of the data which GitHub \textit{API} can provide at a certain date. This great collavorative effort is a huge advantage
...
\begin{itemize}
  \item Login into my account, visit the repository \emph{URL} and press the \textbf{fork} button on GitHub's web interface.
...
\begin{itemize}
  \item \textbf{Private repository}. Some repositories in GitHub can be private hiring a plan to do so.
  The only solution is to perform the request with a token with granted access permissions to that repository,
  otherwise private repositories are ignored.
...
The approach that have been followed to apply this filtering is to define three types of heuristics, related to
file extensions and keywords:
\begin{itemize}
  \item \textbf{Level-1} file extensions, those which are a top priority in our search. They are inmediately considered
   as positive results.
  \item \textbf{Level-2} file extensions, marked as relevant only if they meet one or more additional conditions.
  \item \textbf{Key-words}, specific words or groups of characters which a filename with a \textit{Level-2} extension
  has to contain to be considered as a positive result.
\end{itemize}
From all \texttt{JSON} files which have been obtained before, we are interested only in those included into
\textbf{trees} directory. Each of those files contains the file structure of the last version of its repository,
...
\begin{itemize}
  \item Load \texttt{JSON} data into a \emph{Python} \textit{dictionary}.
  \item Obtain the \textit{parent} \texttt{tree} object.
  \item Check the \texttt{type} field for every \textit{child} object.
  \item If its type is ``\texttt{blob}'', check if it matches with the pre-defined patterns and heuristics.
...
Here is an example of one \texttt{URL} which belongs to a \texttt{blob} object (1) converted to the \texttt{URL} which points
to the actual file (2):
\begin{lstlisting}[language=bash]
